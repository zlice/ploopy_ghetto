...
bool pmw_spi_init(void) {
    ...
    //spi_write_adv(REG_Angle_Tune, constrain(ROTATIONAL_TRANSFORM_ANGLE, -15, 15));

    // WORKING !?!?!
    // myne - jacking this for liftoff config
    spi_write_adv(REG_Lift_Config, 0x0);
    wait_ms(1);
    spi_write_adv(REG_LiftCutoff_Tune_Timeout, 0x20);
    wait_ms(1);
    //spi_write_adv(REG_Angle_Snap, 0x1);
    spi_write_adv(REG_LiftCutoff_Tune_Min_Length, 0x0);
    wait_ms(1);
